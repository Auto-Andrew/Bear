...
void Widget::on_voteSubmit_clicked()//提交选择
{
    ...
    if(flag==1)//选村长
    {
        ...
        if(currentNO==n+1)//选村长结束->进入红/黑夜
        {
            ...
            ui->textBrowser->insertPlainText("\n"+QString::number(maxIndex)+"号玩家成为村长 投票时拥有"+QString::number(n%2==1?0.5:1.5)+"票\n");
...
void Widget::on_random_clicked()
{
    ...
    int t=rand()%1024;
    ui->textBrowser->insertPlainText(QString::number(currentNO)+"号玩家抽到了随机数："+QString::number(t)+"\n");
...
{
    ui->submit->show();
    ui->textBrowser->insertPlainText(QString::number(k)+"有什么遗言？\n\n");
